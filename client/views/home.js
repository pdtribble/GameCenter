...
// ── Shared card/view styles (injected once) ───────────────────────────────────
...
export function renderHome(container, socket, state, navigate) {
  ...
  container.innerHTML = `
    ...
    ">
      ...
      ">
        ...
        ">MULTIPLAYER</div>
        <div id="mp-online-pill" style="
          font-family:var(--gc-mono,'DM Mono',monospace);
          font-size:0.62rem;
          color:var(--gc-green,#30d890);
          background:rgba(48,216,144,0.08);
          border:1px solid rgba(48,216,144,0.18);
          border-radius:20px;
          padding:3px 10px;
        ">— online</div>
      </div>
      ...
      <!-- Lobby panel (shown when a game is selected) -->
  ...
  const onlinePill   = container.querySelector('#mp-online-pill');
  ...
  let gameConfigs     = {};
  let gameMeta        = {};
  let currentPanelGame = null;  // gameType currently shown in lobby panel
  ...
  fetch('/api/me/stats')
    ...
    .then(data => {
      ...
      onlinePill.textContent = `${data.onlineCount ?? 0} online`;
    })
    .catch(() => {});
  ...
  function loadLobbies() {
    fetch('/api/lobbies')
      ...
      .then(data => {
        allLobbies = data || [];
        updateGameBadges();
        if (currentPanelGame) renderLobbyList(currentPanelGame);
      })
      .catch(() => { allLobbies = []; });
  }
  loadLobbies();
  ...
  function showLobbyPanel(gameType, gameLabel) {
    ...
    panelTitle.textContent = gameLabel || gameType;
    ...
    for (const lobby of filtered) {
      ...
      item.innerHTML = `
        <div style="flex:1;min-width:0">
          <div style="color:var(--gc-text,#f0f0f8);font-family:var(--gc-font,'Anybody',sans-serif);font-size:0.88rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">${escHtml(lobby.hostName)}'s Lobby</div>
          <div style="color:var(--gc-muted,rgba(240,240,248,0.4));font-family:var(--gc-mono,'DM Mono',monospace);font-size:0.65rem;margin-top:2px">Code: ${escHtml(lobby.joinCode)}</div>
        </div>
        <div style="display:flex;align-items:center;gap:8px;flex-shrink:0">
          ...
          <button class="gc-card-join-btn${isFull ? ' watch' : ''}"
            data-join-code="${escHtml(lobby.joinCode)}"
            data-spectate="${isFull}">
            ${isFull ? 'Watch' : 'JOIN'}
          </button>
        </div>`;

      item.querySelector('.gc-card-join-btn').addEventListener('click', e => {
        e.stopPropagation();
        openJoinModal(lobby.joinCode, isFull);
      });
  ...
  // Auto-open join modal if prefill code in URL
  if (prefillCode) {
  ...
  function openCreateModal(defaultGameType) {
    ...
    backdrop.querySelector('#cm-submit').addEventListener('click', () => {
      ...
      socket.emit('lobby:create', {
        gameType:    gt,
        playerName:  backdrop.querySelector('#cm-name').value.trim(),
        pin:         backdrop.querySelector('#cm-pin').value,
